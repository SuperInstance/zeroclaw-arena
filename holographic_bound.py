...
def run_experiment():
    ...
    print("\n[4] Binary search for minimum 95% threshold...")
    lo, hi = 10, N
    best_min = N
    ...
    for iteration in range(10):
        ...
        trial_wrs = []
        for trial in range(5):
            random.seed(1000 + iteration * 10 + trial)
            subset_keys = set(random.sample(all_keys, min(mid, N)))
            reconstructed = reconstruct_field(full_field, subset_keys)
            wr, _, _ = evaluate_win_rate(reconstructed, n_games=500)
            trial_wrs.append(wr)
        
        avg_wr = np.mean(trial_wrs)
        pct = avg_wr / full_wr * 100 if full_wr > 0 else 0
        
        print(f"    iter={iteration}: size={mid}, WR={avg_wr:.3f} ({pct:.1f}%)")
